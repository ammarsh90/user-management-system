...
@section('title', 'تعديل إعداد تلغرام: ' . $setting->name)
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.telegram.update', $setting->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('chat_id') is-invalid @enderror" id="chat_id" name="chat_id" value="{{ old('chat_id', $setting->chat_id) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('event_type') is-invalid @enderror" id="event_type" name="event_type" required>
                        ...
                        @foreach($eventTypes as $value => $label)
                            <option value="{{ $value }}" {{ old('event_type', $setting->event_type) == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('event_type')
                        <div class="invalid-feedback">{{ $message }}</div>
